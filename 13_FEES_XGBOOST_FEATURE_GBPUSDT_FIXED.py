...
class LSTMXGBoostBot:
    def __init__(self):
        ...
        lookback_bars = 260  # 200 + 60 for LSTM
        klines = self.fetch_klines(limit=lookback_bars)
        
        if len(klines) >= lookback_bars:
            self.train_models(klines)
            self.models_trained = True
            print("ML models trained and ready")
        else:
            print(f"Insufficient data for training: {len(klines)}/{lookback_bars}")
