...
class Strategy5_EMARSIBot:
    ...
    def calculate_indicators(self, df):
        ...
        # Proper crossover detection with state tracking
    ...
    def should_close(self):
        ...
        profit_pct = ((current_price - self.entry_price) / self.entry_price * 100 if is_long 
        ...
        if self.ema_divergence > self.config['trail_divergence'] and profit_pct > 0:
            if not self.trailing_stop:
                self.trailing_stop = (current_price * (1 - self.config['stop_loss'] / 100) if is_long
                                     else current_price * (1 + self.config['stop_loss'] / 100))
            else:
                if is_long:
    ...
    def log_trade(self, action, price, info=""):
        if action in ["BUY", "SELL"] and not self.current_trade_id:
            stop_loss_price = (price * (1 - self.config['stop_loss']/100) if action == "BUY"
                             else price * (1 + self.config['stop_loss']/100))
            take_profit = (price * (1 + self.config['take_profit']/100) if action == "BUY"
                          else price * (1 - self.config['take_profit']/100))
            ...
            expected_exit = price
            actual_exit = self.apply_slippage(price, "SELL" if self.position.get('side') == "Buy" else "BUY", "limit")
    ...
    async def execute_trade(self, signal):
        ...
        stop_loss_price = (signal['price'] * (1 - self.config['stop_loss']/100) if signal['action'] == 'BUY'
                          else signal['price'] * (1 + self.config['stop_loss']/100))
    ...
    def show_status(self):
        ...
        current_price = float(self.price_data['close'].iloc[-1])
        status_parts = [f"📊 XRP: ${current_price:.4f}", f"Bal: ${self.account_balance:.0f}"]
        
        if self.position:
            side = self.position.get('side', '')
            profit_pct = 0
            if self.entry_price:
                profit_pct = ((current_price - self.entry_price) / self.entry_price * 100 if side == "Buy"
                            else (self.entry_price - current_price) / self.entry_price * 100)
            ...
            status_parts.append(f"⏳ {order_side} @ ${order_price:.4f} ({age}s)")
        else:
            indicators = self.calculate_indicators(self.price_data)
            if indicators:
                status_parts.append(f"RSI: {indicators['rsi']:.1f}")
                status_parts.append("EMA: ↑" if indicators['ema_fast'] > indicators['ema_slow'] else "EMA: ↓")
        
        print(" | ".join(status_parts), end='\r')
    
    async def run_cycle(self):
        # Emergency stop check
        if self.unified_logger.daily_pnl < -self.unified_logger.max_daily_loss:
            print(f"🔴 EMERGENCY STOP: Daily loss ${abs(self.unified_logger.daily_pnl):.2f} exceeded limit")
            if self.position:
